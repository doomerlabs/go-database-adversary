#!/usr/bin/env node
import { createRequire as __goDatabaseCreateRequire } from 'node:module'; const require = __goDatabaseCreateRequire(import.meta.url);
...
var domain = {
  ...
  rules: [
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} multi-row query loop${count === 1 ? " does" : "s do"} not check the row iterator's final error.`,
      whyItMatters: "Next stops for both normal exhaustion and driver errors; only Err distinguishes a complete result from a truncated one.",
...
function goBlockEnd(source, openBrace) {
  let depth = 0;
  let quote;
  let escaped = false;
...
function rowsOwnershipEscapes(scope, escapedVarName) {
...
function buildModelReviewRequestFromDiscovery(change, analysis, files) {
  ...
  return {
    ...
    evidenceById,
    request: {
      prompt: GO_DATABASE_MODEL_PROMPT,
      input,
      schema: GO_DATABASE_MODEL_SCHEMA,
      budget: {
...
async function applyModelDatabaseReview(ctx, output, evidenceById, staticSeverities = [], staticPrimaryConcern) {
  ...
  for (const observation of output.observations.slice(0, MAX_MODEL_OBSERVATIONS)) {
    ...
    ctx.review.observe({
      key: `go-database.model.${observation.id}`,
...
async function runModelDatabaseReview(ctx, analysis, files, staticSeverities = [], staticPrimaryConcern) {
  ...
  try {
    ...
    await applyModelDatabaseReview(
      ctx,
      result.output,
      evidenceById,
      staticSeverities,
...
async function reviewDomain(ctx, analysis, discoveryFiles = []) {
  ...
  const modelStatus = await runModelDatabaseReview(
    ctx,
    analysis,
    discoveryFiles,
    staticSeverities,
